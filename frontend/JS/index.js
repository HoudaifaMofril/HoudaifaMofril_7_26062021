// Affiche les articles postés sur la BDD avec une requete GET

request("posts", 200, "GET", null, [{ key: "Authorization", value: "Bearer " + localStorage.getItem("Token") }])
	.then(function (data) {
	for (let post of data) {
		...
		//Requête fetch() pour récupérer l'utilisateur ADMIN

		fetch('http://localhost:3000/api/auth/users', {method: 'GET'})
			.then(response => response.json())
			.then(userAdmin => {

				if (localStorage.userId == post.userId || userAdmin.userId == localStorage.userId && userAdmin.isAdmin === true) {
